...
var composer = {
	...
	initESClient: function(context, registry, dbName, callback){
		...
		var username = registry.coreDB[dbName].credentials.username || "";
		var password = registry.coreDB[dbName].credentials.password || "";
		...
		if (!context.esKeepAlive || !context.esClient) {
			context.esClient = new elasticsearch.Client(esConfig);
		}

		context.esClient.ping({
		...
		async.forEachOf(context[daemonName].daemonJobs, function (value, key, callback) {
			...
			context[daemonName].daemonJobs[key].wf = function(soajs, next){
				...
				context[daemonName].config.dbs.forEach(function (oneDB) {
					if(oneDB.es){
						composer.initESClient(context[daemonName].es[oneDB.name], soajs.registry, function(error){
							if(error){
	...
	generateService: function (serviceName, cb) {
		...
		var __composerInit = function (req, res, next) {
			...
			context[serviceName].config.dbs.forEach(function (oneDB) {
				...
				if(oneDB.es){
					composer.initESClient(context[serviceName].es[oneDB.name], req.soajs.registry, function(error){
						if(error){
