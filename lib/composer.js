			...
			context[daemonName].daemonJobs[key].wf = function(soajs, next){
					...
					if(error){
						...
						var exitJob = function (soajs, info) {
							context[daemonName].config.dbs.forEach(function (oneDB) {
								var dbName = oneDB.name.replace(/__test__/i, "");
								if(oneDB.es){
				...
				function buildDbConnection(oneDB, cb){
					...
					if(oneDB.es){
						...
						composer.initESClient(context[daemonName].es[dbName], soajs.registry, oneDB.name, soajs.log, cb);
					}
					else if (!oneDB.multitenant) {
						if (!context[daemonName].dbs[dbName]) {
							context[daemonName].dbs[dbName] = new _core.mongo(soajs.registry.coreDB[oneDB.name]);
		...
		var __composerInit = function (req, res, next) {
			...
			function buildDBConnection(oneDB, cb){
				...
				if(oneDB.es){
					...
					composer.initESClient(context[serviceName].es[dbName], req.soajs.registry, oneDB.name, req.soajs.log, cb);
				}
				else if (!oneDB.multitenant) {
					if (!context[serviceName].dbs[dbName]) {
						context[serviceName].dbs[dbName] = new soajs.mongo(req.soajs.registry.coreDB[oneDB.name]);
