...
var composer = {
		...
		async.forEachOf(context[daemonName].daemonJobs, function (value, key, callback) {
			...
			context[daemonName].daemonJobs[key].wf = function(soajs, next){
					...
					if(error){
						...
						var exitJob = function (soajs, info) {
							context[daemonName].config.dbs.forEach(function (oneDB) {
								if(oneDB.es){
									if(!context[daemonName].es[oneDB.name].esKeepAlive){
										context[daemonName].es[oneDB.name].driver.close();
				...
				function buildDbConnection(oneDB, cb){
					if(oneDB.es){
						context[daemonName].es[oneDB.name] = {};
						composer.initESClient(context[daemonName].es[oneDB.name], soajs.registry, oneDB.name, soajs.log, cb);
					}
					else if (!oneDB.multitenant) {
						if (!context[daemonName].dbs[oneDB.name]) {
							context[daemonName].dbs[oneDB.name] = new _core.mongo(soajs.registry.coreDB[oneDB.name]);
						...
						var dbConfig = _core.meta.tenantDB(soajs.registry.tenantMetaDB, oneDB.name, soajs.tenant.code);
						context[daemonName].dbs[oneDB.name] = new _core.mongo(dbConfig);
						soajs.log.debug("creating connection for: ", oneDB.name);
						return cb(null, true);
					}
				}
	...
	generateService: function (serviceName, cb) {
		...
		var __composerInit = function (req, res, next) {
			...
			async.each(context[serviceName].config.dbs, buildDBConnection, function(error){
				...
				res.soajs.returnAPIResponse = function (req, res, info) {
					context[serviceName].config.dbs.forEach(function (oneDB) {
						if(oneDB.es){
							if(!context[serviceName].es[oneDB.name].esKeepAlive){
								context[serviceName].es[oneDB.name].driver.close();
			...
			function buildDBConnection(oneDB, cb){
				if(oneDB.es){
					if(!context[serviceName].es[oneDB.name]){
						context[serviceName].es[oneDB.name] = {};
					}
					composer.initESClient(context[serviceName].es[oneDB.name], req.soajs.registry, oneDB.name, req.soajs.log, cb);
				}
				else if (!oneDB.multitenant) {
