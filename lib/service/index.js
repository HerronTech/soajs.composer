...
var lib = {
	...
	"cleanupConnection1": function (oneDB, context, serviceName, req) {
		var dbName = oneDB.name;
		if (oneDB.es && !context[serviceName].es[dbName].esKeepAlive) {
	...
	"renderOneAPI": function (oneAPISchema, oneAPI, newStyle) {
		if (oneAPISchema.imfv && oneAPISchema.imfv.commonFields) {
			...
			if (Array.isArray(oneAPISchema.imfv.commonFields) && oneAPISchema.imfv.commonFields.length > 0) {
				oneAPI.commonFields = utils.cloneObj(oneAPISchema.imfv.commonFields);
			}
		}
	...
	"loadAndInjectModel": function (context, serviceName) {
		return function (req, res, next) {
			...
			var modelName = req.soajs.servicesConfig.model || req.soajs.config.models.name;
		...
		for (var method in context[serviceName].config.schema) {
			...
			if (methods.indexOf(method) !== -1) {
				...
				for (var api in context[serviceName].config.schema[method]) {
					...
					context[serviceName].serviceAPIs[method][api] = lib.renderOneAPI(context[serviceName].config.schema[method][api], context[serviceName].serviceAPIs[method][api], true);
				}
			}
			else {
				context[serviceName].newStyle = false;
				context[serviceName].serviceAPIs[method] = lib.renderOneAPI(context[serviceName].config.schema[method], context[serviceName].serviceAPIs[method], false);
			}
		}
	},
